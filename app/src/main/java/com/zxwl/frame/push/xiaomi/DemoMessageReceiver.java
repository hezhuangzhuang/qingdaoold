package com.zxwl.frame.push.xiaomi;
...
import com.zxwl.frame.BuildConfig;
import com.zxwl.frame.R;
import com.zxwl.frame.activity.LauncherActivity;
...
public class DemoMessageReceiver
    ...
    public static final String TAG = "DemoMessageReceiver";
...
//        MainActivity.logList.add(0, getSimpleDate() + " " + log);
        ...
        String accountId = PreferenceUtil.getString(context, Constant.SIP_ACCOUNT, LoginCenter.getInstance().getAccount());

        ToastUtil.showShortToast(context, accountId + "");
        if (!TextUtils.isEmpty(accountId)) {
            MsgIOServer.startService(context, accountId);
        }

        Stack<Activity> stack = AppManager.getInstance().getStack();
        Log.i(TAG, "stack-->" + stack.size());

        //进程还在
        if (AppUtils.isFrontProcess(context, BuildConfig.APPLICATION_ID)) {
            MainActivity.startActivity(context);
        } else {
            LauncherActivity.startActivity(context);
        }
...
//        MainActivity.logList.add(0, getSimpleDate() + "    " + log);
    ...
    public void onRequirePermissions(Context context, String[] permissions) {
        ...
        Log.e(DemoMessageReceiver.TAG,
                "onRequirePermissions is called. need permission" + arrayToString(permissions));

        if (Build.VERSION.SDK_INT >= 23 && context.getApplicationInfo().targetSdkVersion >= 23) {
            Intent intent = new Intent();
            intent.putExtra("permissions", permissions);
            intent.setComponent(new ComponentName(context.getPackageName(), PermissionUtils.PermissionActivity.class.getCanonicalName()));
