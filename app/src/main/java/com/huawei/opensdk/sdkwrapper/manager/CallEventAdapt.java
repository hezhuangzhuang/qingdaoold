...
class CallEventAdapt implements TupCallNotify {
     ...
     * 4：无效状态
     *
     * @param tupRegisterResult
     */
    @Override
    public void onRegisterResult(TupRegisterResult tupRegisterResult) {
        PreferencesHelper.saveData(UIConstants.REGISTER_RESULT_TEMP, tupRegisterResult.getRegState());

        //TODO:断网之后会收到的回调
        if (tupRegisterResult.getReasonCode() == 834) {
            //打印日志
            LogUtils.d("onRegisterResult-->华为平台监听到网络断了");
            PreferencesHelper.saveData(UIConstants.REGISTER_RESULT, tupRegisterResult.getReasonCode());
            //如果网络连接成功
        ...
        if (3 != tupRegisterResult.getRegState() && 1 != tupRegisterResult.getRegState()) {
            ...
            try {
                isLogOut = PreferencesHelper.getData(UIConstants.IS_LOGOUT, Boolean.class);
            } catch (Exception e) {
            }
            //没有调用登出接口
            if (!isLogOut) {
